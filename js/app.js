  ...
  try {
    ...
    if (mainLens) {
      lensBlinkInterval = setInterval(() => {
        ...
        mainLens.style.backgroundColor =
          currentBg === "red" ? "#1e90ff" : "red";
      }, 300);
    }
    ...
    if (pokemonData.length > 0) {
      renderSinglePokemon(pokemonData[currentIndex], currentLcdMode);
    ...
    if (screen)
      screen.innerHTML =
        '<div style="color:red; text-align:center; padding-top:40px; font-family:var(--font-primary);">SYSTEM ERROR</div>';
...
function setupControls() {
  ...
  if (btnUp) {
    btnUp.addEventListener("click", () => {
      currentLcdMode = currentLcdMode === "info" ? "stats" : "info";
      renderSinglePokemon(pokemonData[currentIndex], currentLcdMode);
      playBeep(500, 0.05);
    });
  }
  ...
  if (btnPokedex) {
    btnPokedex.addEventListener("click", () => {
      ...
      // Add
      const spriteUrl =
        currentObj.sprites.front_default ||
        `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${currentObj.id}.png`;
      let displayName = currentObj.name.toUpperCase();
      if (currentObj.speciesData) {
        const koName = currentObj.speciesData.names.find(
  ...
  const bgmAudio = document.getElementById("page-bgm");
  if (bgmAudio) {
    bgmAudio.volume = 0.08; // 30% volume
  }
  if (btnBgm && bgmAudio) {
    btnBgm.style.opacity = "1"; // Default on style
    bgmAudio
      .play()
      .then(() => {
        bgmEnabled = true;
    ...
    if (e.key === "Enter") {
      ...
      let index = pokemonData.findIndex((p) => {
        ...
        if (p.speciesData && p.speciesData.names) {
          const koName = p.speciesData.names.find(
            (n) => n.language.name === "ko",
          );
      ...
      if (index !== -1) {
        ...
        const screen = document.getElementById("lcd-screen");
        screen.innerHTML =
          '<div style="color:var(--deep-black); text-align:center; padding-top:40px; font-size:1.2rem; font-family:var(--font-primary);">데이터 검색 중...</div>';
        ...
        if (newPokemon) {
          ...
          // Not found
          screen.innerHTML =
            '<div style="color:red; text-align:center; padding-top:40px; font-size:1.2rem; font-family:var(--font-primary);">검색 실패</div>';
